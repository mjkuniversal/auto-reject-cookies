(function() {
  ...
  const RELOAD_LOOP_KEY = 'arc_reload_count_' + window.location.hostname;
  const RELOAD_LOOP_TIME_KEY = 'arc_reload_time_' + window.location.hostname;
  const MAX_RELOADS = 3;
  const RELOAD_WINDOW_MS = 10000; // 10 seconds

  function checkReloadLoop() {
    const now = Date.now();
    const lastTime = parseInt(sessionStorage.getItem(RELOAD_LOOP_TIME_KEY) || '0');
    let count = parseInt(sessionStorage.getItem(RELOAD_LOOP_KEY) || '0');

    // Reset counter if last reload was more than 10 seconds ago
    if (now - lastTime > RELOAD_WINDOW_MS) {
      count = 0;
    }

    count++;
    sessionStorage.setItem(RELOAD_LOOP_KEY, count.toString());
    sessionStorage.setItem(RELOAD_LOOP_TIME_KEY, now.toString());

    if (count > MAX_RELOADS) {
      console.warn('[Auto Reject Cookies] Reload loop detected on', window.location.hostname, '- disabling for this session');
      return true; // Loop detected
    }
    return false;
  }

  const reloadLoopDetected = checkReloadLoop();
  function log(...args) {
  ...
  // Domain-level tracking: skip re-processing domains where we already rejected cookies
  const HANDLED_DOMAINS_KEY = 'arc_handled_domains';
  const HANDLED_DOMAIN_TTL = 60 * 60 * 1000; // 1 hour
  let handledDomains = {};
  ...
  function isDomainHandled() {
    const domain = getBaseDomain(window.location.hostname);
    const timestamp = handledDomains[domain];
    if (timestamp && (Date.now() - timestamp) < HANDLED_DOMAIN_TTL) {
      return true;
    }
    // Expired entry - clean it up
    if (timestamp) {
      delete handledDomains[domain];
    }
    return false;
  }
  ...
  function markDomainHandled() {
    const domain = getBaseDomain(window.location.hostname);
    handledDomains[domain] = Date.now();
    log('Marking domain as handled:', domain);
    ...
    try {
      ...
      const now = Date.now();
      const cleaned = {};
      for (const [d, t] of Object.entries(handledDomains)) {
        if (now - t < HANDLED_DOMAIN_TTL) {
          cleaned[d] = t;
        }
      }
      handledDomains = cleaned;
      browser.storage.local.set({ [HANDLED_DOMAINS_KEY]: handledDomains });
    } catch (e) {
      log('Error saving handled domains:', e);
    }
  }
  ...
  const CUSTOM_CMP_HANDLERS = {
    // GitHub uses radio buttons for each cookie category instead of a single reject button
    'GitHub': function() {
      // Only run on GitHub domains
      const hostname = window.location.hostname;
      if (!hostname.includes('github.com') && !hostname.includes('github.io')) {
        return false;
      }

      // Find all reject radio buttons
      const rejectRadios = document.querySelectorAll('input[type="radio"][value="reject"]');
      if (rejectRadios.length === 0) return false;

      log('GitHub: Found', rejectRadios.length, 'reject radio buttons');

      let clickedAny = false;
      for (const radio of rejectRadios) {
        if (!radio.checked && !radio.hasAttribute(HANDLED_ATTR)) {
          radio.setAttribute(HANDLED_ATTR, 'true');
          radio.click();
          clickedAny = true;
          log('GitHub: Clicked reject radio for', radio.name);
        }
      }

      if (clickedAny) {
        // Poll for save button to become enabled (GitHub enables it after radio selection)
        let attempts = 0;
        ...
        const trySaveButton = () => {
          attempts++;
          log('GitHub: Attempting to find/click save button (attempt', attempts + ')');

          // GitHub-specific: look for "Save changes" button by aria-label
          let saveButton = document.querySelector('button[aria-label="Save changes"]');
          ...
          if (!saveButton) {
            ...
            for (const btn of buttons) {
              ...
              if (text === 'save changes' || text === 'save' || text === 'save preferences') {
                saveButton = btn;
                break;
              }
            }
          ...
          if (saveButton) {
            log('GitHub: Found save button, disabled:', saveButton.disabled);
            if (!saveButton.disabled && !saveButton.hasAttribute(HANDLED_ATTR)) {
              saveButton.setAttribute(HANDLED_ATTR, 'true');
              saveButton.click();
              console.log('[Auto Reject Cookies] GitHub: Clicked save button');
              return; // Success
            }
          }

          // Retry if button not found or still disabled
          if (attempts < maxAttempts) {
            setTimeout(trySaveButton, pollInterval);
          } else {
            log('GitHub: Save button never became enabled after', maxAttempts, 'attempts');
          }
        };
        ...
        console.log('[Auto Reject Cookies] Rejected via GitHub (radio buttons)');
  ...
  async function checkSettings() {
    try {
      const result = await browser.storage.local.get(['enabled', 'whitelist', HANDLED_DOMAINS_KEY]);
      isEnabled = result.enabled !== false; // Default to enabled
      whitelist = result.whitelist || [];
      handledDomains = result[HANDLED_DOMAINS_KEY] || {};
    } catch (e) {
  ...
  function tryClick(element) {
    ...
    // Save scroll position before clicking - dismissing banners can cause page scroll
    const scrollX = window.scrollX;
    const scrollY = window.scrollY;
    element.click();
    // Restore scroll position immediately and after a delay (for async banner removal)
    window.scrollTo(scrollX, scrollY);
    setTimeout(() => window.scrollTo(scrollX, scrollY), 50);
    setTimeout(() => window.scrollTo(scrollX, scrollY), 200);
    ...
    for (const cmp of KNOWN_CMPS) {
      ...
      for (const selector of cmp.selectors) {
        try {
          ...
          if (element) {
            log(`Found ${cmp.name} element with selector: ${selector}`);
            log(`Element tag: ${element.tagName}, id: ${element.id}, class: ${element.className}`);
            log(`Element text: "${(element.textContent || '').trim().substring(0, 50)}"`);
            if (tryClick(element)) {
              console.log(`[Auto Reject Cookies] Rejected via ${cmp.name} using selector: ${selector}`);
              return true;
            }
          }
        } catch (e) {
    ...
    for (const element of allClickables) {
      ...
      for (const pattern of REJECT_PATTERNS) {
        if (pattern.test(text)) {
          // Only click if in a consent context to avoid clicking unrelated buttons
          if (!isInConsentContext(element)) {
            log('Skipping priority button not in consent context:', text);
            continue;
          }
    ...
    for (const element of allClickables) {
      ...
      for (const pattern of REJECT_PATTERNS) {
        if (pattern.test(text)) {
          // Only click if in a consent context to avoid clicking unrelated buttons
          if (!isInConsentContext(element)) {
            log('Skipping button not in consent context:', text);
            continue;
          }
  ...
  function rejectCookies(allowSettingsClick = false) {
    ...
    if (reloadLoopDetected) {
      log('Skipping - reload loop detected on this site');
      return;
    }
    ...
    if (isDomainHandled()) {
      log('Skipping - domain already handled recently');
      return;
    }
